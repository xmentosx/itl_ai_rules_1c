...
Before the first code or metadata write, run the project/host preflight that
records `quick-fix` or `OpenSpec` in workflow state. This classification is a
mechanical gate, not a reminder the model may silently skip.
...
For a quick-fix, define one focused regression scenario before editing: previous faulty behavior, triggering action, expected fixed result, and verification type. Add a second scenario only for a separate meaningful boundary. A pre-change failing run is useful only when it is cheap and adds diagnostic value; never require two complete executable gates merely to prove the same defect. Project gates in `USER-RULES.md` and the host workflow win. Resolve effective Vanessa/event-log modes before planning or authoring executable tests. An `off` component is not auto-authored or run; it produces explicit skipped/partial evidence and never a normal fresh pass. If the fix cannot be expressed as one local scenario, or it affects architecture, a public API, related metadata, or several behaviors, promote it to OpenSpec. `verificationPolicy=block` still requires full evidence before result/close; `warn` may report only an explicitly confirmed partial implementation, never `verified` or `done`.
